...
def group_detection(target_station, k=3):
    ...
    # plt.xlabel('2016')
    y_train, groups, lbl = synthetic_groups(train_data[target_station], plot=False, alpha=alpha, threshold=2.0)
    ...
    # print "\n---------- Testing data ---------\n"
    try:
        y_train, tgroups, lblt = synthetic_groups(test_data[target_station], plot=False, alpha=alpha, threshold=2.0)
...
def main_test(target_station, save_fig=True):
    ...
    plt.title(target_station)
    plt.xlabel('2016')
    y_train, lbl_train = synthetic_groups(train_data[target_station], True, alpha=ALPHA, fault_type=FAULT_TYPE)

    plt.subplot(212)
    plt.xlabel('2017')
    y_test, lbl_test = synthetic_groups(test_data[target_station], True, alpha=ALPHA, fault_type=FAULT_TYPE)
    if save_fig:
        plt.savefig("plots/" + target_station + ".jpg")
    plt.close()
    ...
    with PdfPages("detectionplot/" + target_station + "plots.pdf") as pdf:
        ...
        "Training accuracy"
        # plt.subplot(323)
        train_result['train_pairwise_vote_auc'], train_result['train_pairwise_vote_pr'] = combine_models(models,
                                                                                                         train_data,
                                                                                                         target_station,
                                                                                                         y_train,
                                                                                                         lbl_train,
...
def cdf_plot(target_station, k=5):
    ## Plot cdf of all stations.

    k_station = nearby_stations(target_station, k=K, path=nearest_location_path)
    k_station = np.intersect1d(k_station, all_stations)

    plt.subplot(1, 2, 1)

    plt.plot(train_data[target_station].cumsum(), '--r', label=target_station)
    for stn in k_station:
        plt.plot(train_data[stn].cumsum(), label=stn)
    plt.legend(loc='best')
    plt.ylabel("cumulative percipitation (mm)")
    plt.xlabel("Days")
...
if __name__ == '__main__':
    ...
    all_stations = list(set(train_data.columns.tolist()).intersection(test_data.columns.tolist()))
    print(all_stations)
    # print all_stations
    ...
    if mode == "single_station":
        ...
        print
        main_test(target_station, save_fig=True)
    elif mode == "K_tuning":
        ...
        for target_station in all_stations[:]:
            ...
            target_station
            dx = pd.DataFrame(all_result)
            dx.to_csv(source + "all_station_k_tunining.csv", index=False)
        ...
        target_station = "TA00025"
        print
        group_detection(target_station)  # for target_station in all_stations]
        plt.show()
        all_result = []
        # with PdfPages("detectionplot/"+source+"_"+str(K)+"_fault_detection.pdf") as pdf:
        #
        #     for target_station in all_stations[:]:
        #         print target_station
        #
        #         res = group_detection(target_station, k=K)
        #         all_result.append(res)
        #         pd.DataFrame(all_result).to_csv(source+"_"+str(K)+"_rfparametric_results_kde.csv",index=False)
        #         plt.title(target_station)
        #         pdf.savefig()
        #         plt.close()
        #         #break
        ...
        with PdfPages("detectionplot/" + source + str(K) + "_cdf_plot_dist.pdf") as pdf:
            ...
            for target_station in all_stations[:]:
                ...
                target_station
                #
                cdf_plot(target_station, k=K)
                pdf.savefig()
                plt.close()
        #     #break
        # cdf_plot(target_station, K)
        # plt.show()
    else:
        print
        util.sample(range(10))
        print
        mode, " Not found"
