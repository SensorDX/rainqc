...
import pandas as pd
from src.common import roc_metric
...
# lreg.fit(X=np.log(x+ src.eps), y=np.log(x+src.eps), sample_weight=glm_fitted)
# lreg_fitted = lreg.predict(np.log(x+src.eps))
# res = (lreg_fitted - np.log(src.eps + y))
#src.residual_plot(np.log(observed_value + src.eps), np.log(y+src.eps), lreg_fitted)
...
#roc_metric( src.log_reg.predict_proba(x)[:,1], y_binary) # debugging
