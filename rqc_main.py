...
class MainRQC:
    ...
    def available_neighbors(self, target_station, from_date, to_date):
        """
        ...
        for stn in nearby_station_list:
            if self.data_source.check_data(stn, self.variable, from_date, to_date):
                available_stations[stn] = self.data_source.get_data(stn, self.variable, from_date, to_date)
                k -=1
            if k<1:
                break
    ...
    def fit(self, date_from, date_to, **kwargs):
        ...
        k_stations = self.data_source.nearby_stations(target_station=self.target_station, radius=self.radius, k=self.num_k_stations)
    ...
    def build_view(self, target_station, date_from, date_to, **kwargs):
        ...
        target_station_data =  self.__fetch_data(target_station, self.variable, date_from, date_to, group=kwargs.get("group"))  #query_data(target_station)
